...
# RHEL 9 / CentOS 9 CIS加固脚本（支持Level选择）
# 基于CIS Red Hat Enterprise Linux 9 Benchmark v1.0.0

set -e

echo "RHEL 9 / CentOS 9 CIS加固脚本"
echo "================================="
...
EOF
    ...
    if [ "$LEVEL" == "2" ]; then
        ...
        if ! grep -q "^/var/log" /etc/fstab; then
            echo "/var/log /var/log xfs defaults,nosuid,nodev,noexec 0 0" >> /etc/fstab
        fi
        
        echo "1.4.1 Level 2: 初始化AIDE..."
        aide --init
...
section_2() {
    ...
    # 2.1 禁用不必要的服务
    echo "2.1 禁用不必要的服务..."
    services_to_disable=("avahi-daemon" "bluetooth" "cups" "dhcpd" "nfs-server" "rpcbind" "smb" "winbind")
    
    # Level 2额外禁用的服务
    if [ "$LEVEL" == "2" ]; then
        services_to_disable+=("httpd" "nginx" "mariadb" "postgresql" "named" "postfix" "sendmail" "vsftpd" "telnet-server" "rsh-server" "talk-server" "ntalk-server" "xinetd")
    fi
    ...
    # 2.2 确保关键服务启用
    echo "2.2 确保关键服务启用..."
    services_to_enable=("auditd" "crond" "sshd" "firewalld" "chronyd")
...
# 8. 配置SELinux
section_8() {
    echo "执行第8节: SELinux配置..."
    
    # 确保SELinux启用
    echo "8.1 确保SELinux启用..."
    sed -i 's/^SELINUX=.*/SELINUX=enforcing/' /etc/selinux/config
    setenforce 1 2>/dev/null || true
    
    # 确保SELinux策略正确
    echo "8.2 确保SELinux策略正确..."
    semanage boolean -l | grep -E '(httpd_can_sendmail|httpd_can_network_connect|httpd_read_user_content)'
}
