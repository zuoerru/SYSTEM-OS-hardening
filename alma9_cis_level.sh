...
# AlmaLinux 9 CIS加固脚本
# 支持Level 1和Level 2选择
# 基于CIS AlmaLinux 9 Benchmark
# 基于官方RHEL9-CIS仓库（AlmaLinux 9与RHEL 9二进制兼容）

set -e

echo "AlmaLinux 9 CIS加固脚本"
echo "==================================="
echo "基于官方ansible-lockdown/RHEL9-CIS仓库"
echo "AlmaLinux 9与RHEL 9二进制兼容"
echo ""
...
echo ""
echo "开始AlmaLinux 9 CIS Level $LEVEL加固..."
echo "==================================="
...
detect_os() {
    if [ -f /etc/redhat-release ]; then
        if grep -q "AlmaLinux release 9" /etc/redhat-release; then
            echo "检测到AlmaLinux 9系统"
            return 0
        else
            echo "警告: 此脚本专为AlmaLinux 9设计"
            echo "但会尝试在当前系统上执行"
            return 0
        fi
    else
...
main() {
    ...
    echo ""
    echo "AlmaLinux 9 CIS Level $LEVEL加固完成！"
    echo "==================================="
    echo "基于官方ansible-lockdown/RHEL9-CIS仓库"
    echo "AlmaLinux 9与RHEL 9二进制兼容"
    echo ""
